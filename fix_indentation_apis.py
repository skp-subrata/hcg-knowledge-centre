...
# Replace the previous unindented code if present
if "# --- SERVER-SIDE PAGINATION & SEARCH ADMIN APIS ---" in content:
    import re
    content = re.sub(r'# --- SERVER-SIDE PAGINATION & SEARCH ADMIN APIS ---[\s\S]*?(?=\t@app\.route\(\'/api/v1/releases/active|\n\t@app\.route\(\'/api/v1/releases/active)', '', content)

content = content.replace('\t@app.route(\'/api/v1/releases/active\', methods=[\'GET\'])', endpoints_code + '\n\t@app.route(\'/api/v1/releases/active\', methods=[\'GET\'])')

with open('d:/apps/app.py', 'w', encoding='utf-8') as f:
    f.write(content)

print("SUCCESS: Fixed indentation for admin API endpoints!")
