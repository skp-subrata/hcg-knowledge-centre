...
def test_active_release_is_1_9_0_and_earlier_releases_are_retired(db):
	active = db("SELECT version_number, features, improvements, bug_fixes FROM app_releases WHERE is_active = 1")
	assert len(active) == 1 and active[0]["version_number"] == "1.9.0"
	...
	assert {"1.8.0", "1.7.0", "1.6.1", "1.5.0", "1.4.0", "1.3.2"} <= retired
