...
| QA-030 | uploads | Every file upload (profile picture, post thumbnail, attachments) failed with `FileNotFoundError` on a fresh checkout or empty disk because `uploads/` is git-ignored and was never created | app.py `upload_path()` → **fixed** (folder created on demand at all five save sites; tests/test_uploads.py deletes the folder before uploading) |
