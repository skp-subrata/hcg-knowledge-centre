...
# I will just find everything from const userId =  down to </script> and replace it
pattern = r'const userId = document\.getElementById\(\'interest-container\'\)\.dataset\.userid;.*?</script>'

script_block = '''const userId = document.getElementById('interest-container').dataset.userid;
...
function renderChips() {
    ...
    activeInterests.forEach(i => {
        ...
        chip.innerHTML = 
            ...
            <button type="button" class="hover:text-indigo-900 dark:hover:text-white rounded-full p-0.5 transition-colors focus:outline-none" onclick="removeInterest()">
...
content = re.sub(pattern, script_block, content, flags=re.DOTALL)
